...
class YOLOWriter:
    ...
    def add_bnd_box(self, x_min, y_min, x_max, y_max, name, difficult, info = None):
    ...
    def save(self, class_list=[], target_file=None, info_items_list = None):
        ...
        for box in self.box_list:
            # class_index, x_center, y_center, w, h = self.bnd_box_to_yolo_line(box, class_list, info_items_list = info_items_list)
            
            class_index, x_center, y_center, w, h, info = self.bnd_box_to_yolo_line(box, class_list, info_items_list = info_items_list)
            # print (classIndex, x_center, y_center, w, h)

            # thinkman edit here
            # out_file.write("%d %.6f %.6f %.6f %.6f\n" % (class_index, x_center, y_center, w, h))
            out_file.write("%d %.6f %.6f %.6f %.6f %d %d %d %d\n" \
                           % (class_index, x_center, y_center, w, h, info[0], info[1], info[2], info[3]))
...
class YoloReader:
    ...
    def __init__(self, file_path, image, class_list_path=None, info_items_list = None):
        # shapes type:
    ...
    def add_shape(self, label, info, x_min, y_min, x_max, y_max, difficult):

        points = [(x_min, y_min), (x_max, y_min), (x_max, y_max), (x_min, y_max)]
        self.shapes.append((label, info, points, None, None, difficult, 0))
    ...
    def yolo_line_to_shape(self, class_index, info_index_list, x_center, y_center, w, h):
        label = self.classes[int(class_index)]
        ...
        return label, info, x_min, y_min, x_max, y_max
    ...
    def parse_yolo_format(self):
        ...
        for bndBox in bnd_box_file:
            ...
            class_index, x_center, y_center, w, h, *info_index_list = bndBox.strip().split(' ')
            label, info, x_min, y_min, x_max, y_max = self.yolo_line_to_shape(class_index, info_index_list, x_center, y_center, w, h)

            # Caveat: difficult flag is discarded when saved as yolo format.
            self.add_shape(label, info, x_min, y_min, x_max, y_max, False)
